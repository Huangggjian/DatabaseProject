        ...
        flush_thread_ = new thread([&]
                                   {
                                       ...
                                       { //The thread is triggered every LOG_TIMEOUT seconds or when the log buffer is full
                                           unique_lock<mutex> latch(latch_);
                                           // (2) When LOG_TIMEOUT is triggered.
                                           cv_.wait_for(latch, LOG_TIMEOUT, [&]
                                                        { return needFlush_.load(); }); //最多等0.1s。阻塞当前线程，直到条件变量被唤醒或在指定的超时持续时间之后
                                           assert(flushBufferSize_ == 0);               //
                                           if (logBufferOffset_ > 0)
                                           {
                                               std::swap(log_buffer_, flush_buffer_);         //log需要刷盘。交换2个指针
                                               std::swap(logBufferOffset_, flushBufferSize_); //交换大小
                                               disk_manager_->WriteLog(flush_buffer_, flushBufferSize_);
                                               flushBufferSize_ = 0; //全部fflush完。设置为0
 ...
 */
    ...
    {
        unique_lock<mutex> latch(latch_);
        if (logBufferOffset_ + log_record.GetSize() >= LOG_BUFFER_SIZE) //1.满了
        {
            needFlush_ = true;
            cv_.notify_one(); //let RunFlushThread wake up.
            appendCv_.wait(latch, [&]
                           { return logBufferOffset_ + log_record.GetSize() < LOG_BUFFER_SIZE; }); //唤醒刷盘线程，直到不满
        }
        log_record.lsn_ = next_lsn_++;                                               //当前日志编号
        memcpy(log_buffer_ + logBufferOffset_, &log_record, LogRecord::HEADER_SIZE); //包含必须要有的20字节信息
        int pos = logBufferOffset_ + LogRecord::HEADER_SIZE;                         //写日志的起始位置

        if (log_record.log_record_type_ == LogRecordType::INSERT)
        {
            memcpy(log_buffer_ + pos, &log_record.insert_rid_, sizeof(RID)); //写入insert rid
            pos += sizeof(RID);                                              //移位
            // we have provided serialize function for tuple class
            log_record.insert_tuple_.SerializeTo(log_buffer_ + pos); // 继续写入tuple
        }
    ...
    void LogManager::Flush(bool force)
    {
        ...
        {
            needFlush_ = true;
            cv_.notify_one(); //let RunFlushThread wake up.  唤醒刷盘函数RunFlushThread
            if (ENABLE_LOGGING)
                appendCv_.wait(latch, [&]
                               { return !needFlush_.load(); }); //block append thread 等待刷完（needflush变为false）
        }
        else
        {
