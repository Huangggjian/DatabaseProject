 ...
 */
    Page *BufferPoolManager::FetchPage(page_id_t page_id)
    {
        ...
        Page *tar = nullptr;
        if (page_table_->Find(page_id, tar))    //如果从hash table找不到，说明已经落盘了。所有要找一个新的页（从freelist或者
       // LRU,同时在磁盘里把落盘的内容重新读取进来）
        { //1.1
            tar->pin_count_++;
            replacer_->Erase(tar);
            return tar;
        ...
        if (tar->is_dirty_)
        {
            ...
            if (ENABLE_LOGGING && log_manager_->GetPersistentLSN() < tar->GetLSN())//GetPersistentLSN:上一次持久化到磁盘的日志id
        ...
        //4
        disk_manager_->ReadPage(page_id, tar->data_);   //已经落盘啦！从磁盘都进来以前的
        tar->pin_count_ = 1;
        tar->is_dirty_ = false;
 ...
 */
    Page *BufferPoolManager::NewPage(page_id_t &page_id)
    {
        ...
        {
            /*
                LSN不仅存在于redo log中，还存在于数据页中，在每个数据页的头部，
                有一个fil_page_lsn记录了当前页最终的LSN值是多少。通过数据页中的LSN值和redo log中的LSN值比较，如果页中的LSN值小于redo log中LSN值，
                则表示数据丢失了一部分，这时候可以通过redo log的记录来恢复到redo log中记录的LSN值时的状态。
