...
namespace cmudb
{
    ...
    {
        memcpy(storage, &size_, sizeof(int32_t)); //storage后面4字节写入size_的内容。然后再写入data_的内容
        memcpy(storage + sizeof(int32_t), data_, size_);
    }

    void Tuple::DeserializeFrom(const char *storage)    //开始指针
    {
        uint32_t size = *reinterpret_cast<const int32_t *>(storage);        //前4字节存放size
        // construct a tuple
        this->size_ = size;
        if (this->allocated_)   
            delete[] this->data_;
        this->data_ = new char[this->size_];        
        memcpy(this->data_, storage + sizeof(int32_t), this->size_);//size后面的数据放进来
