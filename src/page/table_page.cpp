...
namespace cmudb
{
    ...
    /**
 * Header relatedlog record (include/logging/log_record.h)
 ...
 */
                                ...
                                LogManager *log_manager)
    {
        ...
        SetTupleOffset(i, GetFreeSpacePointer()); //偏移
        SetTupleSize(i, tuple.size_);             //大小
        if (i == GetTupleCount())
        {
            rid.Set(GetPageId(), i);
 ...
 *markdelete : 假装删除，把tuple size变为负数（其他数据不能用这个空间）， Rollbackdelete则把负数重新变为正数（其他操作可以对他操作了）
 */
                                ...
                                LogManager *log_manager)
    {
        ...
        for (int i = 0; i < GetTupleCount();
             ++i)
        { // update tuple offsets (including the updated one)
