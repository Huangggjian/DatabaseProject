 ...
 */
    ...
    {
        assert(GetSize() > 1);
        int st = 1, ed = GetSize() - 1; //第二个key作为左下标，因为第一个key是invaild
        while (st <= ed)
        ...
        return array[st - 1].second; //array[st-1].first <= key <= array[st].first
    }
 ...
 */
        ...
        BufferPoolManager *buffer_pool_manager)
    {
        ...
        {
            //internal 的下标不是应该从1开始？ (ind 0放就放着吧，正好internal page也不需要，无效的。)
            recipient->array[i - copyIdx].first = array[i].first;
            recipient->array[i - copyIdx].second = array[i].second;
            //新split出来的new page, 因为是internal page type，所以旧page的后半部分child要重新设置parent pageid
 ...
 */
        ...
        BufferPoolManager *buffer_pool_manager)
    {
        ...
        parent->SetKeyAt(parent->ValueIndex(GetPageId()), array[0].first); //更新parent page中的index(因为本页最前面的已经放到左边一页的最右边了，所以父page的index处的value要改了(改成第二个数的))
 ...
 */
    ...
    {
        assert(GetSize() + 1 < GetMaxSize());
        memmove(array + 1, array, GetSize() * sizeof(MappingType));//后移1
        IncreaseSize(1);
        array[0] = pair;
        // update child parent page id
        page_id_t childPageId = pair.second;
