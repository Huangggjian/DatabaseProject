 ...
 * For EACH log record, HEADER is like (5 fields in common, 20 bytes in totoal)  每条记录都要包含这20字节的内容
 *-------------------------------------------------------------
 * | size | LSN | transID | prevLSN | LogType |
 *-------------------------------------------------------------
 * For insert type log record											insert 记录
 *-------------------------------------------------------------
 * | HEADER | tuple_rid | tuple_size | tuple_data(char[] array) |
 *-------------------------------------------------------------
 * For delete type(including markdelete, rollbackdelete, applydelete)	delete记录
 *-------------------------------------------------------------
 * | HEADER | tuple_rid | tuple_size | tuple_data(char[] array) |
 *-------------------------------------------------------------
 * For update type log record											update记录
 *------------------------------------------------------------------------------
 * | HEADER | tuple_rid | tuple_size | old_tuple_data | tuple_size |
 * | new_tuple_data |
 *------------------------------------------------------------------------------
 * For new page type log record											new page记录
 *-------------------------------------------------------------
...
namespace cmudb
{
	...
	private:
		...
		RID delete_rid_;        //日志的record id(64位，高32位是pageid，低32位是slot num)
		Tuple delete_tuple_;
